...
class ShoulderScaptionAnalyzer:
    def __init__(self):
        self.reset()
    ...
    def process(self, landmarks_3d, worker):
        ...
        left_angle  = calculate_angle_3d(landmarks_3d[23], landmarks_3d[11], landmarks_3d[15])
        right_angle = calculate_angle_3d(landmarks_3d[24], landmarks_3d[12], landmarks_3d[16])
        avg_angle   = (left_angle + right_angle) / 2

        is_down   = avg_angle > 155.0
        is_raised = avg_angle < 110.0
        ...
        if self.stage == "WAITING":
            ...
            if not is_down:
                ...
                self.frame_count = 0
                self.hit_top = False
                worker.system_status.emit("● RECORDING — RAISE ARMS DIAGONALLY", "#ff4444")
                speak_async("Begin.")
            ...
            try:
                if len(self.buffer) >= 5 and MODEL_GLOBAL:
                    ...
                    with timed("bilstm::Shoulder Scaption"):
                ...
                print(f"[ShoulderScaption] Inference error: {e}")
            self.stage = "WAITING"
            worker.system_status.emit("ARMS AT YOUR SIDES", "#ffaa00")
