...
class PhysioDashboard(QMainWindow):
    ...
    def __init__(self, username: str, token: str, parent=None):
        ...
        self.setWindowIcon(QIcon(os.path.join(FRONTEND_DIR, "Vision_favicon.png")))
...
def run_application():
    ...
    # Tell Windows to use our icon in the taskbar instead of the Python icon.
    # Must be called before QApplication is created.
    if os.name == "nt":
        import ctypes
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID("PhysioVision.App")

    app = QApplication(sys.argv)

    # Setting the icon on the QApplication propagates it to every window
    # in the process (login window, dashboard, any future dialogs).
    _icon_path = os.path.join(FRONTEND_DIR, "Vision_favicon.png")
