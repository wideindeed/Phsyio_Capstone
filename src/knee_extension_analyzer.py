...
# Knee angle thresholds for seated knee extension
DOWN_THRESHOLD = 110.0   # knee bent (seated start position)
UP_THRESHOLD   = 150.0   # knee extended (raised position)
...
class KneeExtensionAnalyzer:
    """
    State machine for Seated Knee Extension.
    ...
    def process(self, landmarks_3d, worker) -> None:
        ...
        # Average knee angle from both legs (hip-knee-ankle)
        left_angle  = calculate_angle_3d(landmarks_3d[23], landmarks_3d[25], landmarks_3d[27])
        right_angle = calculate_angle_3d(landmarks_3d[24], landmarks_3d[26], landmarks_3d[28])
        avg_angle   = (left_angle + right_angle) / 2.0

        is_down   = avg_angle < DOWN_THRESHOLD   # legs bent at start
        is_raised = avg_angle > UP_THRESHOLD     # leg(s) extended outward
        ...
        if self.stage == "WAITING":
            ...
            if not is_down:
                ...
                self.frame_count = 0
                self.hit_top     = False
                worker.system_status.emit("● RECORDING (EXTEND LEG)", "#ff4444")
                speak_async("Begin.")
            ...
            try:
                if len(self.buffer) >= 5 and MODEL_GLOBAL:
                    ...
                    with timed("bilstm::Knee Extension"):
                ...
                print(f"[KneeExtension] Inference error: {e}")
